...
async function resolveConfigBranchId(branchIdOrName) {
  ...
  try {
    const byId = await getDb().ref(`branchConfigurations/${branchIdOrName}`).once("value");
    if (byId.exists()) return branchIdOrName;
    const all = await getDb().ref("branchConfigurations").once("value");
    ...
    console.error("[sms] failed to resolve branch id:", err.message);
  }
  return null;
}
...
async function getSmsConfiguration(branchIdOrName) {
  try {
    ...
    const snap = await getDb().ref(`systemConfiguration/${branchId}/sms`).once("value");
    if (snap.exists()) {
    ...
    try {
      await getDb().ref(`systemConfiguration/${branchId}/sms`).update({
        ...parsed,
        updatedAt: Date.now(),
      });
    } catch (seedErr) {
      console.error("[sms] failed to seed branch SMS config:", seedErr.message);
    }
    return parsed;
  } catch (err) {
    console.error("[sms] failed to load SMS configuration:", err.message);
