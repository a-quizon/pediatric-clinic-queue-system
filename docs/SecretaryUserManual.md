...
On a phone, Validate Reservation is shortened to **Validate**, and Manage Queue is shortened to **Queue**.
...
It only works after you or the doctor starts the queue for today (see [Start the queue](#start-the-queue)). If no queue is running, you will see **No Active Queue For** your branch.
...
- **Total Active** — how many people are waiting or with the doctor
...
Tap a patient row to see parent contact details (or walk-in details, including optional parent name and phone if they were entered).
...
3. Choose a **schedule** from the dropdown (date, hours, and how many slots are left). The list includes **Upcoming** published schedules and queues already **Active Now** or **Paused**. Closed or ended clinic days do not appear.
4. Optional: type **Parent's Name** and **Parent's Phone** (10 digits after +63). Use this for phone-in bookings so the clinic can text them. Leave blank for in-person walk-ins.
...
The patient appears in Manage Queue with a **Walk-in** tag. On an Active Now or Paused queue, they join **at the back of the live line** (they get the next ticket number and wait their turn). They do not skip people already waiting.

If you entered a phone:

- **Upcoming** schedule → they get the usual reservation-details text.
- **Active Now** or **Paused** queue → they get the “active queue” text (queue number and position).

That phone is used **only for this booking text**. Walk-ins do not get later texts such as queue started, near turn, penalty, or forfeiture.
...
Check-in still works while the queue is **paused**, and after it is **closed to new bookings** (existing tickets stay valid).

1. Open **Validate Reservation**.
2. The camera usually starts by itself. Point it at the parent’s QR code, **or** type their **6-character code**.
...
If the camera does not start, click **Start Camera**. Allow camera access if the browser asks. Click **Stop Camera** if you need to turn it off.
...
| Clinic Queue Not Started | Start today’s queue from Schedules first |
| Queue Closed | The clinic day already ended (not the lock button) |
...
1. Open **Manage Queue**.
2. Find the waiting row with the **Penalize** button. It appears on:
   - the first parent who is **not checked in**, or
   - a **walk-in** who is first in line (even though walk-ins are already checked in).
...
- **Parent app bookings:** if they check in (QR/code) before time runs out, they stay in line at the new place and the timer stops.
- **Walk-ins:** they are already checked in, so they cannot scan again to stop the timer. Send them to the doctor before time runs out, or they lose the slot.
...
Do not cancel a walk-in who is already **with the doctor**. Finish that visit first.
...
These buttons sit at the top of **Manage Queue**. The doctor has the same four actions on Queue.

| Button | What it does |
| --- | --- |
| Pause (pause icon) | Temporarily stop the live session. Check-in can still happen. |
| Play (play icon) | Resume after a pause. |
| Lock (lock icon) | Close the queue to **new** reservations. People already in line keep their place. Consultations continue. You can still check in existing tickets. You **cannot** add new walk-ins. |
| Stop (square icon) | End the clinic day. Only works when **nobody is waiting and nobody is with the doctor**. |

Closing the queue **cannot be undone**. After you lock it, the pause/play buttons go away. Only **End Clinic Session** remains. Parents cannot book that schedule again.
...
You only create schedules for your assigned branch. Clinic hours are filled in automatically from that branch’s opening and closing times. You cannot type different hours here.
...
3. Confirm the **branch** (locked to yours).
4. Pick the **clinic date**.
5. Check that **Clinic Hours** appear.
6. Type **slot capacity**.
...
Drafts can also be **Edit**ed or **Delete**d.
...
After the queue is running (or closed to new bookings), click **Manage Queue** on the card to jump back to the desk screen.
...
The page shows which branch you are configuring. Save Queue Rules and SMS Settings **separately**.
...
These are the text messages parents receive. Walk-in phone numbers are **not** used for these later messages (only the one booking text in [Adding a Walk-in Patient](#3-adding-a-walk-in-patient)).

| Setting | What it controls |
| --- | --- |
| **Near Turn — Patients Ahead** (1–10) | Send the “your turn is near” text when this many people (or fewer) are still ahead. Each reservation gets this text only once. |
...
Gray buttons like `{queueNumber}` insert that detail into the message. A character count appears under each message. Click **Save SMS Settings**.
...
First screen after login. Shows today’s published schedule, who is with the doctor, counts (Remaining Queue / Waiting / Checked In / Completed), and recent activity.

On a **wide computer screen**, a compact Manage Queue also appears here. On a phone, use the **Queue** tab instead.

*[Screenshot: Dashboard]*

### Request Check-In

On Manage Queue, click **Request Check-In** to remind the next parent who is not yet checked in to come to the desk. There is a short wait (about 30 seconds) before you can send another reminder.
...
A large screen for the waiting room: **Now Serving** and **Next In Queue** (queue numbers only, no names). It also shows the branch name and the current time. Up to 10 waiting numbers are listed.

1. On Manage Queue, click **Live Queue Monitor**.
2. Optionally click the fullscreen button.

Paused and closed queues show a banner at the top (**Queue Paused** or **Queue Closed**).
