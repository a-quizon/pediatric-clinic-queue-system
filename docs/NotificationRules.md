...
| **SLOT_RESERVED** | Confirms a finalized reservation (in-app / push / SMS with reservation details). Phone-in walk-ins get SMS only (no Notification Center) when created before the queue starts. | Parent clicks Save Information (`patientInfoCompleted`), or secretary creates a walk-in with `parentPhone` while queue is `not_started`. | Specific Parent (or walk-in phone) |
