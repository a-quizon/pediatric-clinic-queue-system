...
| **NEARING_TURN** | Patient first reaches N or fewer slots ahead of their turn (SMS once per reservation via `nearTurnSmsSent`; N from that branch’s `systemConfiguration/{branchId}/sms/nearingTurnAheadCount`, default 3). | Queue Engine Recalculation | Specific Parent |
